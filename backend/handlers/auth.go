...
func RegisterHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Save to database
	_, err := db.DB.Exec("INSERT INTO users (id, email, password, name, points) VALUES ($1, $2, $3, $4, $5)",
		user.ID, user.Email, user.Password, user.Name, 0)
...
func LoginHandler(w http.ResponseWriter, r *http.Request) {
	...
	var user models.User
	err := db.DB.QueryRow("SELECT id, email, name, points FROM users WHERE email = $1 AND password = $2",
		creds.Email, creds.Password).Scan(&user.ID, &user.Email, &user.Name, &user.Points)
